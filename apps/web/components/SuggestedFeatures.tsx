...
const SuggestedFeatures = () => {
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold text-text-light mb-4">Suggested Features</h2>
      <div className="space-y-4">
        {features.map((feature) => (
          <motion.div
            key={feature.id}
            variants={featureVariants}
            className="flex items-center bg-background-medium/30 p-4 rounded-xl border border-border-subtle cursor-pointer hover:bg-primary-700/50 transition-colors duration-200"
          >
            <feature.icon className="w-8 h-8 text-accent-main mr-4" />
            <div>
              <p className="font-semibold text-text-light text-lg">{feature.name}</p>
