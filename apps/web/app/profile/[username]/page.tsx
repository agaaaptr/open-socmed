...
export default function ProfileViewPage({ params }: { params: { username: string } }) {
  ...
  const fetchFollowData = useCallback(async (type: 'followers' | 'following', userId: string) => {
    // Only fetch if data is not already loaded or if explicitly requested to refresh
    // For this refactor, we assume initial fetch covers all, so this is for tab switching
  ...
  const handleTabChange = (tab: string) => {
      setActiveTab(tab);
      // Data is already fetched on initial load, so no need to re-fetch here
      // unless we implement lazy loading for tabs later.
  };
