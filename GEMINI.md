...
- [x] **Checkpoint 2.4: Frontend Core Feature Implementation (In Progress)**
  ...
  - [x] **Profile Followers/Following Display:** Fixed the bug where the followers/following list displayed "API returned invalid data" when the list was empty. The `fetchFollowData` function in `apps/web/app/profile/[username]/page.tsx` now correctly handles empty array responses from the backend, and the `UserList` component displays a more informative message (e.g., "No followers yet." or "Not following anyone yet.") in such cases.
...
- [x] **Follow/Unfollow API:** Implement endpoints to create and delete follow relationships (`POST /api/follow`, `DELETE /api/follow`).
