...
- [x] **Checkpoint 2.4: Frontend Core Feature Implementation (In Progress)**
  ...
  - [x] **Notification Feature:** Implemented comprehensive notification display, mark as read functionality, and unread count badges on UI.
  ...
  - [x] **Mobile Post Modal Fixes:** Resolved issues where the on-screen keyboard would cover the input field and where closing the modal from pages other than home would cause an incorrect redirect.
...
- [x] **Loading State Enhancement:** Implemented a centralized `LoadingState` component with animated spinner and text, replacing previous loading indicators across `home/page.tsx`, `profile/[username]/page.tsx`, `profile/page.tsx`, `search/page.tsx`, `settings/profile/page.tsx`, `components/CreatePost.tsx`, `components/FollowButton.tsx`, and `components/Timeline.tsx` for consistent and professional UI. The `LoadingState` component now supports different loader types (spinner and dots) to accommodate various UI needs. Fixed inconsistent button loading animations by adjusting `LoadingState` styling and button content wrapping to maintain consistent button dimensions during loading states.
  - [x] **Loading State Overhaul:** Completely refactored the `LoadingState` component for better performance and aesthetics. Replaced the default spinner with a smoother, more fluid `CircleDashed` icon animated by Framer Motion. Reworked all buttons with loading states (Follow, Post, Save, etc.) to prevent layout shifts and maintain size consistency. Introduced a `size` prop for better reusability.
...
- [x] **Checkpoint 3.1: Backend Core Feature Implementation (Completed)**
  ...
  - [x] **Notification Feature:** Implemented database schema, API endpoints for fetching, creating (via follow), and marking notifications as read.
...
- [x] **Search Feature `forEach` Error:** Fixed the "can't access property 'forEach' of null" error in the search feature (`apps/web/app/search/page.tsx`) by adding a check to ensure `followingData` is an array before attempting to iterate over it. This prevents the error when the backend returns `null` for an empty following list.
...
- [x] **Conditional Profile Actions:** Fixed a bug where "Edit Profile" and "Sign Out" buttons were visible on other users' profiles. These actions are now only displayed on the authenticated user's own profile page. Also, relocated the "Back to Home" button to the top-right for better UI consistency.
